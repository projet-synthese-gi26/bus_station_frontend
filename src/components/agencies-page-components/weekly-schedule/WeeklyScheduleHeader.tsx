...
interface WeeklyScheduleHeaderProps {
  ...
  // BLOC 4 : nouveau bouton "Générer la semaine"
  onGenerateWeekRequest?: () => void;
}
  ...
  return (
    <div className="bg-gray-800 text-white p-4 rounded-t-2xl flex flex-col md:flex-row justify-between items-center gap-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-300">
            Gérez les voyages de la semaine
          </p>
        </div>
      </div>
      <div className="flex flex-wrap items-center justify-center gap-2">
        {allCategories.map((cat) => {
          ...
          return (
            <button
              ...
              className={`px-3 py-1 text-xs sm:text-sm font-bold rounded-full transition-all duration-200 text-white ${bgColor} ${
                isActive
        ...
        {onGenerateWeekRequest && (
          <button
            onClick={onGenerateWeekRequest}
            className="flex items-center gap-2 px-3 py-1 text-xs sm:text-sm font-bold rounded-full transition-all duration-200 bg-amber-500 text-white hover:bg-amber-600 border-2 border-transparent"
            title="Générer les voyages d'une semaine à partir de ce planning"
          >
...
// END OF FILE: src/components/agencies-page-components/weekly-schedule/WeeklyScheduleHeader.tsx
