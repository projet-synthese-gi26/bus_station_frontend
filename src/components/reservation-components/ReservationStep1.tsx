...
export default function ReservationStep1({
  ...
  onContinue,
  totalPrice,
}: ReservationStep1PropsInterface) {
  // Capacité totale du bus : priorité au véhicule, sinon nbrPlaceReservable
  const busCapacity = tripDetails.vehicule?.nbrPlaces ?? tripDetails.nbrPlaceReservable ?? 0;
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full">
      ...
      <div className="w-full lg:w-1/2 p-6 bg-white lg:h-full lg:overflow-y-auto">
        ...
        <p className="italic text-md mb-6 text-gray-600">
          Please choose the seat(s) you want to book
        </p>
        ...
        {/* Plan de bus selon capacité totale */}
        {busCapacity === 56 && (
        ...
        {![56, 70, 75, 80].includes(busCapacity) && (
          <div className="flex flex-col items-center justify-center py-12 text-center text-gray-400">
            <span className="text-4xl mb-3">🚌</span>
            <p className="font-medium text-gray-600">Plan de sièges non disponible</p>
            <p className="text-sm mt-1">
              Capacité du bus : {busCapacity > 0 ? `${busCapacity} places` : "non renseignée"}
            </p>
          </div>
        )}
      </div>
