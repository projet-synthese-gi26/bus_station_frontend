  ...
  if (hook.isLoading) {
    return (
      <div className="space-y-6">
        <PageHeader
          title={
            hook.tripId && hook.tripName
              ? `Réservations - ${hook.tripName}`
              : "Gestion des Réservations"
          }
          subtitle={
            hook.tripId
              ? "Réservations pour ce voyage spécifique"
      ...
      <div className="p-4">
        ...
        {hook.searchTerm && (
          <p className="mt-3 text-sm text-gray-600">
            {hook.filteredBookings.length} résultat(s) pour &#34;
            {hook.searchTerm}&#34;
          </p>
        )}
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {hook.filteredBookings.map((booking, index) => (
                <BookingTableRow
                  ...
                  booking={booking}
                  index={index}
                  statusInfo={hook.getStatusInfo(
                    booking.reservation?.statutReservation || "RESERVER",
                  )}
