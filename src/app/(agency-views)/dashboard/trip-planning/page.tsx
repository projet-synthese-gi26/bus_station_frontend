...
/**
 * P-20 · Formulaire de planification / création de voyage
 * Route : /dashboard/trip-planning
 * Fichier : app/(agency-views)/dashboard/trip-planning/page.tsx
 *
 * Modes :
 *   ?mode=new            → nouveau brouillon
 *   ?edit={voyageId}     → modifier voyage existant
 *   ?draft={id}          → compléter un brouillon
...
function Textarea({
  ...
  return (
    <textarea
      {...props}
      className={`
        w-full px-3 py-2.5 rounded-xl border text-sm transition-all outline-none resize-none
        ${
          error
...
function Select({
  ...
  return (
    <select
      {...props}
      className={`
        w-full px-3 py-2.5 rounded-xl border text-sm transition-all outline-none bg-white
        ${
          error
            ? "border-red-300 bg-red-50 focus:border-red-400"
    ...
    <div className="space-y-6">
          ...
          {gares.length > 0 ? (
            <Select {...register("pointDeDepart")}>
              ...
              {gares.map((g: GareOption) => (
                <option key={g.idGare} value={g.nom}>
                  {g.nom}
                  {g.ville ? ` — ${g.ville}` : ""}
                </option>
              ))}
        ...
        <Field label="Point d'arrivée (gare)">
          {gares.length > 0 ? (
            ...
            <Input
              ...
              placeholder="ex: Gare de Bonabéri"
            />
          )}
        </Field>
      </div>
      ...
      <Field label="Durée estimée">
        <Input
          ...
          placeholder="ex: 4h30 ou PT4H30M"
        />
      </Field>
    </div>
  );
...
function Step2({
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <Field
          label="Nombre de places réservables"
          required
        ...
        >
          <div className="relative">
            ...
            <Input
              type="number"
              min={1}
              {...register("nbrPlaceReservable", { valueAsNumber: true })}
              className="pl-9"
              error={errors.nbrPlaceReservable?.message}
            />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <Field label="Image miniature (URL)">
          <Input
            {...register("smallImage")}
            placeholder="https://..."
            error={errors.smallImage?.message}
          />
          {watch("smallImage") && (
            <img
              src={watch("smallImage")}
              alt="Aperçu"
          ...
          <Field label="Véhicule">
            ...
            {vehicules.length === 0 && (
              <p className="text-xs text-amber-500 mt-1 flex items-center gap-1">
                <AlertCircle className="w-3 h-3" /> Aucun véhicule trouvé
              </p>
            )}
          </Field>
              ...
              <Select {...register("classVoyageId")} className="pl-9">
                ...
                {classes.map((c) => (
                  <option key={c.idClassVoyage} value={c.idClassVoyage}>
                    {c.nom} — {c.prix.toLocaleString()} XAF
                  </option>
                ))}
              </Select>
            </div>
              ...
              {AMENITIES_LIST.map((a) => {
                ...
                return (
                  <button
                    ...
                    className={`
                      flex flex-col items-center gap-2 p-3 rounded-xl border-2 text-xs font-medium
    ...
    <div className="space-y-5">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-600 mt-1">
              Ce voyage sera sauvegardé comme brouillon{" "}
              <strong>INCOMPLET</strong> : {missingResources.join(", ")} non
              renseigné(s).
            </p>
          </div>
        </div>
      ...
      {mode === "draft" && brouillonStatut === "PRET" && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex gap-3">
          ...
          <p className="text-sm text-green-700 font-medium">
            Brouillon <strong>PRÊT</strong> — vous pouvez le publier
            directement.
          </p>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100 overflow-hidden">
        ...
        <div className="px-4 py-2">
          ...
          {row(
            "Prix",
            values.prix ? `${values.prix.toLocaleString()} XAF` : undefined,
          )}
      ...
      <div className="bg-gradient-to-r from-blue-700 to-indigo-800 text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
          <Link
            ...
            className="inline-flex items-center gap-2 text-blue-200 hover:text-white text-sm mb-5 transition-colors"
          >
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <>
                ...
                {currentStep === 3 && (
                  <Step4
                    ...
                    classes={classes}
                    mode={mode}
                    brouillonStatut={brouillonStatut}
                  />
                )}
              </>
            )}
          ...
          <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-between items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              {/* Toujours proposer "Sauvegarder brouillon" */}
              {isLastStep && (
                <button
                  type="button"
                  onClick={saveDraft}
              ...
              {!isLastStep ? (
                ...
                <button
                  ...
                  title={
                    !canPublish
                      ? "Le brouillon doit être PRÊT pour être publié"
                      : undefined
                  }
                  className="
                    flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold
