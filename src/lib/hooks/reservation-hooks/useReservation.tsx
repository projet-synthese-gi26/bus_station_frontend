...
export function useReservation(
  ...
  const totalLuggage: number = useMemo((): number => {
    return passengersData.reduce(
      (total: number, passenger: PassengerFormType) => {
  ...
  const reservationDetails = [
    { icon: FaHouse, label: "Travel agency", value: tripDetails?.nomAgence },
    {
      icon: FaCalendar,
      label: "Travel date",
      value: formatDateOnly(tripDetails?.dateDepartPrev || ""),
    },
    {
      icon: MapPin,
      label: "Departure location",
      value: tripDetails?.lieuDepart,
    },
    { icon: MapPin, label: "Arrival Location", value: tripDetails?.lieuArrive },
    {
      icon: FaMoneyBill,
      label: "Unit price",
      value: tripDetails?.prix + " FCFA",
    },
  ];
  ...
  async function saveCreatedReservation(
